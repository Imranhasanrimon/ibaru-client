...
import OurBatchOrganizer from "./OurBatchOrganizer";
import OurAllBatchStudents from "./OurAllBatchStudents";

const OurBatch = () => {
    const { user, isLoading: isLoadingAuth } = useAuth()
    const batchNo = user && getStudentId(user.email).slice(0, 2) - 15;
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 space-y-4 ">
                ...
                <Card className="p-4">
                    <CardContent className="p-0 space-y-2 text-center">
                        ...
                        {achievements.map((item, idx) => (
                            <span key={idx} variant="secondary" className="text-gray-500">{item}</span>
                        ))}
                    </CardContent>
                </Card>
            </div>
            ...
            <Card className="p-4 mb-8">
                <CardContent className="p-0 grid grid-cols-2 lg:grid-cols-4 gap-4" >

                    <div className="col-span-2 lg:col-span-4 flex justify-between items-center">
            ...
            <OurBatchOrganizer programChairman={programChairman} coordinator={coordinator} CRInfo={CRInfo} user={user} />

            {/* allStudents */}
            <OurAllBatchStudents allStudents={allStudents} user={user} batchNo={batchNo} />
        </div >
    );
};

export default OurBatch;